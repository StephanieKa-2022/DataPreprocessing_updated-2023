...
# And updated by Stephanie
# resourcecols = ["resource column name 1", "resource column name 2", "resource column name #"]
# used when there are single rows with multiple timestamp columns that
# can be broken down into multiple (distinct) events
import pandas as pd

def transposeColumnsToEventLog(dataset, events, case_id_col_name="id", format="%d/%m/%Y %H:%M", resourcecols=None):
    dataset = ArrangeRows(dataset, [case_id_col_name])
    event_log = []

    for index, row in dataset.iterrows():
        for event_name, timestamp_col in events.items():
            event_entry = {
                'case_id': row[case_id_col_name],
                'event': event_name,
                'timestamp': pd.to_datetime(row[timestamp_col], format=format)
            }

            if resourcecols is not None:
                for resourcecol in resourcecols:
                    event_entry[resourcecol] = row[resourcecol]

            event_log.append(event_entry)
            
    result_df = readPanda(pd.DataFrame(event_log), case_id_col_name, 'event', 'timestamp', format) 
    
    # Drop the last column named "new_time" 
    if 'new_time' in result_df.columns: 
        result_df = result_df.drop(columns=['new_time'])
...
# WHERE CHANGE OCCURED
# keep last event in each sequence of consecutive events in each trace of logs
def keepLastEventSort(dataset):
    dataset=dataset.sort_values(by=['case_id','timestamp'],ascending=False)
    res = pd.DataFrame([])
    grouped2 = dataset.groupby('case_id')
    for name, group in grouped2:
        test = group.loc[group['event'].ne(group['event'].shift())]
        res = pd.concat([res, test])
    return res.sort_values(by=['case_id','timestamp'])
...
#Added by Stephanie
"""
    Keep the last occurrence of a specified event for each case_id and retain other events unsorted.

    Parameters:
    - df: pandas DataFrame
    - event_to_keep: str, the event to keep the last occurrence of

    Returns:
    - pandas DataFrame, modified DataFrame with the last occurrence of the specified event for each case_id
"""
def keep_last_occurrence_of_event(df, event_to_keep):

    # Convert 'timestamp' column to datetime if not already
    #df['timestamp'] = pd.to_datetime(df['timestamp'])

    # Create a mask to identify rows with the specified event
    mask_event = df['event'] == event_to_keep

    # Identify the last occurrence of the specified event for each case_id
    last_occurrences = df[mask_event].groupby('case_id').tail(1)

    # Keep the identified last occurrences and other events
    result_df = pd.concat([df[~mask_event], last_occurrences], ignore_index=True)
